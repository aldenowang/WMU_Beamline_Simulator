...
void RunAction::EndOfRunAction(const G4Run* run)
{
  ...
  const auto generatorAction = static_cast<const IonPrimaryGeneratorAction*>(
  ...
  // write itself in case that ever changes independently.
  if (IsMaster() && G4VVisManager::GetConcreteInstance() == nullptr) {
    const auto now = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
    std::tm tmNow{};
    localtime_s(&tmNow, &now);
    std::ostringstream timestamp;
    timestamp << std::put_time(&tmNow, "%Y%m%d_%H%M%S") << "_n" << nofEvents;

    std::ostringstream foilFileName;
