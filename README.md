\page IonBeamSim Ion Beam Simulator
 ...
 gold foil target (Rutherford scattering), measured by a PPAC and Faraday
 cup. It started from the Geant4 basic/B1 example template; the notes
 below still describe the underlying Geant4 mechanics that carried over.
...
## HOW TO RUN
   ...
   - Execute ionBeamSim in the 'interactive mode' with visualization
```
% ionBeamSim
...
```
   ...
   - Execute ionBeamSim in the 'batch' mode from macro files
   (without visualization)
```
% ionBeamSim run2.mac
% ionBeamSim ionBeamSim.in > ionBeamSim.out
```
